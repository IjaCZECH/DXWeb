...
namespace DX.Data.Xpo.Identity
{
...
#endif
    {
...
#if (NETSTANDARD2_1)
        public async virtual Task AddToAlienRoleAsync(TUser user, string roleName, int alienID,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            await AddToAlienRoleAsync(user, roleName, alienID);
        }
        ...
        public async virtual Task RemoveFromAlienRoleAsync(TUser user, string roleName, int alienID,
    CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            await RemoveFromAlienRoleAsync(user, roleName, alienID);
        }
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await GetAlienRolesAsync(user, alienID);
        }
        ...
        public async virtual Task<bool> IsInAlienRoleAsync(TUser user, string roleName, int alienID,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await IsInAlienRoleAsync(user, roleName, alienID);
        }
        ...
        public async virtual Task<IList<int>> GetAliensAsync(TUser user, string roleName,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await GetAliensAsync(user, roleName);
        }
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await GetAlienUsersInRoleAsync(roleName, alienID);
        }
        ...
        public virtual async Task<IList<TUser>> GetAlienUsersInRoleAsync(string roleName, int alienID)
        {
                ...
                .ExecuteAsync(
                    (db, wrk) =>
                    {
                        ...
                        foreach (XpoDxAlienUserRole alien in new XPCollection(wrk, typeof(XpoDxAlienUserRole),
                         CriteriaOperator.Parse("Role.NormalizedName == ? AND AlienID == ?", roleName, alienID),
                         new SortProperty("Name", SortingDirection.Ascending)))
                        {
                            TUser usr = this.Mapper.CreateModel((alien).User as TXPOUser);
                            users.Add(usr);
                        }
        ...
        public async virtual Task AddToAlienRoleAsync(TUser user, string roleName, int alienID)
        {
            ...
            await DB.ExecuteAsync((db, wrk) =>
            {
                ...
                {
#if (NETSTANDARD2_1)
                    var role = wrk.FindObject(typeof(XpoDxRole),
                        CriteriaOperator.Parse("(NormalizedName == ?) AND (NOT (Aliens[User.Id == ?] AND Aliens[AlienID == ?]))", roleName, u.Id, alienID)) as XpoDxRole;
#else
					var role = u.Session.FindObject(typeof(XpoDxRole),
						CriteriaOperator.Parse("(NameUpper == ?) AND (NOT Aliens[User.ID == ? AND AlienID == ?])", r, u.Id, alienID)) as XpoDxRole;
#endif
                    if (role == null)
                        throw new InvalidOperationException($"Role {roleName} was not found");
                    u.AliensList.Add(new XpoDxAlienUserRole(u.Session) { Role = role, AlienID = alienID });
        ...
        public async virtual Task<IList<int>> GetAliensAsync(TUser user, string roleName)
        {
            ...
            var result = await DB.ExecuteAsync((db, wrk) =>
            {
                ...
                XPCollection<XpoDxAlienUserRole> collection = new XPCollection<XpoDxAlienUserRole>(wrk, CriteriaOperator.Parse("Role.NormalizedName == ? AND User.Id == ?", roleName, userId), null);
                return collection.Select(x => x.AlienID).Distinct().ToList();
            }, false);
            return result;
        }
        ...
        public async virtual Task<IList<string>> GetAlienRolesAsync(TUser user, int alienID)
        {
            ...
            var result = await DB.ExecuteAsync((db, wrk) =>
            {
                ...
                foreach (XpoDxAlienUserRole alien in new XPCollection(wrk, typeof(XpoDxAlienUserRole),
                         ...
                         new SortProperty("Role.Name", SortingDirection.Ascending)))
                {
        ...
        public async virtual Task<bool> IsInAlienRoleAsync(TUser user, string roleName, int alienID)
        {
            ...
            {
#if (NETSTANDARD2_1)
                var alien = wrk.FindObject(typeof(XpoDxAlienUserRole),
                         CriteriaOperator.Parse("Role.NormalizedName == ? AND User.Id == ? AND AlienID == ?", roleName, user.Id, alienID)) as XpoDxAlienUserRole;
#else
				var alien = wrk.FindObject(typeof(XpoDxAlienUserRole),
						 CriteriaOperator.Parse("Role.NameUpper == ? AND User.Id == ? AND AlienID == ?", roleName.ToUpperInvariant(), user.Id, alienID)) as XpoDxAlienUserRole;
#endif
                return (alien != null);
            }, false);
            return result;
